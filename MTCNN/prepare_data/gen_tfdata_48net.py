...
def main():
    ...
    cur_ = 0
    pos_keep = npr.choice(len(pos), size=2000000, replace=True)
    sum_ = len(pos_keep)
    print('Writing')
    for i in pos_keep:
        view_bar(cur_, sum_)
    ...
    cur_ = 0
    neg_keep = npr.choice(len(neg), size=300000, replace=True)
    sum_ = len(neg_keep)
    for i in neg_keep:
        view_bar(cur_, sum_)
        cur_ += 1
    ...
    print('\n'+'part random cropped')
    cur_ = 0
    part_keep = npr.choice(len(part), size=100000, replace=True)
    sum_ = len(part_keep)
    for i in part_keep:
        view_bar(cur_, sum_)
        line = part[i]
