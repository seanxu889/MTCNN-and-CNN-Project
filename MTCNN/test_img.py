        ...
        with tf.Graph().as_default():
            ...
            with tf.Session(config=config) as sess:
                ...
                for k in range(0, facenum):
                    crop_img = img[int(rectangles[k][1]):int(rectangles[k][3]), int(rectangles[k][0]):int(rectangles[k][2])]
                    crop_img = cv2.resize(crop_img, (32,32))
                    cv2.imwrite(str(k) + 'crop.jpg', crop_img) # save the each resized faces to current directory, 32*32 RGB
